...
const ownerIds = [8100399804]; // contoh chat_id owner 
    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "XEVORZEX");
...
<blockquote><b>「 Xsigvard 」</b></blockquote>
࿇ ᴀᴜᴛᴏʀ : @fc1sepz
࿇ ᴛɪᴘᴏ  : 1.0 VipBuyOnly
࿇ sᴄʀɪᴘᴛ  : Database Xevorze 
...
`;
  ...
  await ctx.replyWithPhoto(
    { url: "https://files.catbox.moe/i0690f.jpg" },
    {
      caption: teks,
      parse_mode: "HTML",
  ...
  await ctx.replyWithAudio(
    { url: "https://files.catbox.moe/o0plkz.m4a" }, // Ganti dengan URL audio yang diinginkan
    {
      caption: "🎵 Everything u are",
      parse_mode: "HTML"
    }
  );
});
    ...
    const keyboard = {
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "[ INFO XEVORZE APPS ]",
              url: "https://t.me/twbands"
            }
          ]
        ]
      }
...
app.get("/", (req, res) => {
  const filePath = path.join(__dirname, "SONATA", "Login.html");
...
app.get("/execution", (req, res) => {
  try {
    ...
    const filePath = "./SONATA/Login.html";
...
└────────────────────────┘
  `));
        ...
        if (batch < maxBatches) {
          console.log(chalk.yellow(`( Grade XEVORZE 🍂 777 ).`));
          count = 0;
          batch++;
...
const executionPage = (
  ...
  const filePath = path.join(__dirname, "SONATA", "SONATA.html");
  ...
  try {
    ...
    console.error("Gagal membaca file SONATA.html:", err);
